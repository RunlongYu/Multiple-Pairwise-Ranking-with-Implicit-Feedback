    ...
    public static void train() throws FileNotFoundException {
        ...
        for(int iter = 0; iter < num_iterations; iter++) {
            for(int iter_rand = 0; iter_rand < n; iter_rand++) {
                ...
                // sample item i: $i$, h: $p$, o: $p^'$ from $I_u^+$
	        	...
	        	int r_o = (int) Math.floor(Math.random() * itemSet.size());
	        	int i = itemList.get(r_i);         // i.e.   $i$
	        	int h = itemList.get(r_h);         // i.e.   $p$
	        	int o = itemList.get(r_o);         // i.e.   $p^'$
                // sample item j: $j$, z: $q$, g: $q^'$ from $I_u^*$, i.e. implicit feedback
                ...
                float r_succ_u = lambda * (r_ui - r_uj + r_ug - r_uz) + (1-lambda) * (r_uz - r_ug + r_uo - r_uh);
                ...
                // update $U_{u\cdot}$
                for (int f=0; f<d; f++) {
	    			U[u][f] = U[u][f] - eta * ( loss_succ_u * ( lambda * (V[i][f] - V[j][f] - V[z][f] + V[g][f]) + (1 - lambda) * (V[z][f] - V[g][f] - V[h][f] + V[o][f])) + alpha_u * U[u][f] );
	    		}

                // update $V_{i\cdot}$
	        	for (int f=0; f<d; f++) {	
                 V[i][f] = V[i][f] - eta *  ( loss_succ_u * lambda * U[u][f]  + alpha_v * V[i][f] );
                }

                // update $V_{j\cdot}$
                for (int f=0; f<d; f++) {
                    V[j][f] = V[j][f] - eta * ( loss_succ_u * (-1) * lambda * U[u][f] + alpha_v * V[j][f] );
                }

                // update $V_{q\cdot}$
                for (int f=0; f<d; f++) {
	    			V[z][f] = V[z][f] - eta * ( loss_succ_u * (1-2*lambda) * U[u][f] + alpha_v * V[z][f] );
	    	    }

                // update $V_{q^'\cdot}$
	        	for (int f=0; f<d; f++) {	
	    			V[g][f] = V[g][f] - eta * ( loss_succ_u * (2*lambda-1) * U[u][f] + alpha_v * V[g][f] );
	    	   	}
                
                // update $V_{p\cdot}$
                for (int f=0; f<d; f++) {	
	    			V[h][f] = V[h][f] - eta * ( loss_succ_u * (lambda-1) * U[u][f] + alpha_v * V[h][f] );
	    	   	}
                
                // update $V_{p^'\cdot}$
	        	for (int f=0; f<d; f++) {	
	    			V[o][f] = V[o][f] - eta * ( loss_succ_u * (1-lambda) * U[u][f] + alpha_v * V[o][f] );
	    	   	}

                // update bias
                biasV[i] = biasV[i] - eta * ( loss_succ_u * lambda + beta_v * biasV[i] );
	    		biasV[j] = biasV[j] - eta * ( loss_succ_u * (-lambda) + beta_v * biasV[j] );
	    		biasV[h] = biasV[h] - eta * ( loss_succ_u * (lambda-1) + beta_v * biasV[h] );
	    		biasV[o] = biasV[o] - eta * ( loss_succ_u * (1-lambda) + beta_v * biasV[o] );
                biasV[z] = biasV[z] - eta * ( loss_succ_u * (1 - 2 * lambda) + beta_v * biasV[z] );
                biasV[g] = biasV[g] - eta * ( loss_succ_u * (2 * lambda - 1) + beta_v * biasV[g] );
            }
        }
    }
